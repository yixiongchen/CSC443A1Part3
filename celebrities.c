...
int main(int argc, char **argv){
	...
	int result = get_celebrities (&manager, buffer_capacity);
...
int compare_heap (const void *one, const void *two) {
	OutputHeapElement a = *(OutputHeapElement*)one;
	OutputHeapElement b = *(OutputHeapElement*)two;
	if (a.diff > b.diff){
		return 1;
	}
...
int get_celebrities (JoinManager * manager, int buffer_capacity){	
	...
	result = result_outdegree || result_indegree;
	...
	else if (result_indegree==SUCCESS || result_outdegree==SUCCESS){
		result = SUCCESS;
	}
	
	
	while (result == 0) { //Not finish reading both files
...
int print_output_heap (JoinManager * manager){
	...
	qsort(manager->output_heap, manager->current_output_heap_size, sizeof(OutputHeapElement), compare_heap);
	for (int i = manager->current_output_heap_size-1; i >= 0; i--){
		printf("UID=%d， in_degree=%d, out_degree=%d, diff=%d\n",
			manager->output_heap[i].UID, manager->output_heap[i].in_degree,
			manager->output_heap[i].out_degree, manager->output_heap[i].diff);
	}
