...
void merge_sort(char* file_uid1, char* file_uid2){
    ...
    //first read into main memory
    int result_1 = fread(buffer_uid1, sizeof(Record), num_records_chunk, fp_read_uid1);
    int result_2 = fread(buffer_uid1, sizeof(Record), num_records_chunk, fp_read_uid1);
    //iteration
    while(1){

    	//when can not read any records, stop while loop
    	if(uid2_pointer == result_1){
    	...
    	//when can not read any records, stop while loop 
    	if(uid2_pointer == result_2){
    		result_2 = fread(buffer_uid1, sizeof(Record), num_records_chunk, fp_read_uid1);
    		if(result_2 < 0){
    			//flush output buffer to disk
    			fwrite (output, sizeof(Record), output_num_records, fp_write);
    			break;
    	...
    	int next_move = compare_two_uids(buffer_uid1);

    	//compare two edges
    	if(next_move == success){
